...
class LevelPlayRewardVideoVC: BasicUIViewController {
    ...
    @objc private func buttonShow() {
        guard let ad = rewardedAd, ad.isAdReady() else {
            ...
            showAutoDismissTip(message: NSLocalizedString("tip_message", comment: ""))
            return
        }
    ...
    private func loadAd() {
        rewardedAd = LPMRewardedAd(adUnitId: AdsConfig.LevelPlay_Reward_Ad_Id)
...
extension LevelPlayRewardVideoVC: LPMRewardedAdDelegate {
    ...
    func didRewardAd(with adInfo: LPMAdInfo, reward: LPMReward) {
        print("\(TAG) didRewardAd: \(reward.name) x\(reward.amount)")
    }
