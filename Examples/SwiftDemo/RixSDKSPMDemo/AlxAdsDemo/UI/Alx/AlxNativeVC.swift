...
class AlxNativeVC: BaseUIViewController {
    ...
    private var selectedNativeAdId = AdConfig.Alx_Native_Ad_Id
    private lazy var nativeAdIds: [String] = {
        let ids = AdConfig.Alx_Native_Ad_Ids.filter { !$0.isEmpty }
        return ids.isEmpty ? [AdConfig.Alx_Native_Ad_Id] : ids
    }()

    private var isLoading = false
    private var nativeLoader: AlxNativeAdLoader?
    // 注意：nativeAd 需用属性持有，避免被系统回收导致无法展示
    private var pendingNativeAd: AlxNativeAd?
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        if !nativeAdIds.contains(selectedNativeAdId) {
            selectedNativeAdId = nativeAdIds[0]
        }
        navigationItem.title = NSLocalizedString("alx_native_loader", comment: "")
        setupBackground()
        setupUI()
    }
    ...
    private func setupUI() {
        ...
        selectIdButton = createButton(title: "Select Native ID", action: #selector(selectNativeId))
        ...
        idLabel.text = selectedNativeAdId
    ...
    @objc private func loadAd() {
        ...
        isLoading = true
        pendingNativeAd = nil
        logMessage("\(TAG) loading... id: \(selectedNativeAdId)")

        nativeLoader = AlxNativeAdLoader(adUnitID: selectedNativeAdId)
        ...
        nativeLoader?.loadAd(request: req)
    }
    ...
    @objc private func showAd() {
        ...
        let renderVC = AlxNativeRenderVC(nativeAd: nativeAd)
        renderVC.onEvent = { [weak self] event in
    ...
    @objc private func selectNativeId() {
        let titleSpacer = "\n\n\n\n\n\n\n\n"
        let alert = UIAlertController(
            title: "Select Native ID\(titleSpacer)",
            message: nil,
        ...
        if let selectedIndex = nativeAdIds.firstIndex(of: selectedNativeAdId) {
        ...
        alert.addAction(UIAlertAction(title: "Confirm", style: .default, handler: { [weak self] _ in
            ...
            guard self.nativeAdIds.indices.contains(row) else { return }
            self.selectedNativeAdId = self.nativeAdIds[row]
            self.idLabel.text = self.selectedNativeAdId
            self.pendingNativeAd = nil
            self.logMessage("\(self.TAG) selected native id: \(self.selectedNativeAdId)")
        }))
...
extension AlxNativeVC: UIPickerViewDataSource, UIPickerViewDelegate {
    func numberOfComponents(in pickerView: UIPickerView) -> Int {
        return 1
    }

    func pickerView(_ pickerView: UIPickerView, numberOfRowsInComponent component: Int) -> Int {
        return nativeAdIds.count
    }
    ...
    func pickerView(_ pickerView: UIPickerView, viewForRow row: Int, forComponent component: Int, reusing view: UIView?) -> UIView {
        ...
        label.text = nativeAdIds.indices.contains(row) ? nativeAdIds[row] : ""
        return label
    }
}
...
extension AlxNativeVC: AlxNativeAdLoaderDelegate {
    func nativeAdLoaded(didReceive ads: [AlxNativeAd]) {
        logMessage("\(TAG) nativeAdLoaded")
        isLoading = false

        guard let ad = ads.first else {
            logMessage("\(TAG) loaded empty ad list")
            return
        }
