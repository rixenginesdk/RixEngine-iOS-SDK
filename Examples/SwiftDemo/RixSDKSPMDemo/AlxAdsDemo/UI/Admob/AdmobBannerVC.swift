...
extension AdmobBannerVC: BannerViewDelegate {
    func bannerViewDidReceiveAd(_ bannerView: BannerView) {
        isLoading = false
        logMessage("\(TAG) bannerViewDidReceiveAd")
        buttonShow()
    }

    func bannerView(_ bannerView: BannerView, didFailToReceiveAdWithError error: Error) {
        isLoading = false
        logMessage("\(TAG) didFailToReceiveAd: \((error as NSError).code): \(error.localizedDescription)")
    }

    func bannerViewDidRecordImpression(_ bannerView: BannerView) {
        logMessage("\(TAG) bannerViewDidRecordImpression")
    }

    func bannerViewDidRecordClick(_ bannerView: BannerView) {
        logMessage("\(TAG) bannerViewDidRecordClick")
    }

    func bannerViewWillPresentScreen(_ bannerView: BannerView) {
        logMessage("\(TAG) bannerViewWillPresentScreen")
    }

    func bannerViewWillDismissScreen(_ bannerView: BannerView) {
        logMessage("\(TAG) bannerViewWillDismissScreen")
    }

    func bannerViewDidDismissScreen(_ bannerView: BannerView) {
        logMessage("\(TAG) bannerViewDidDismissScreen")
    }
}
