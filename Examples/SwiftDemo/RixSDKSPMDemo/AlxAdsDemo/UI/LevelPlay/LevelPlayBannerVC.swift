...
class LevelPlayBannerVC: BaseUIViewController {
    ...
    @objc private func buttonLoad() {
        ...
        let banner = LPMBannerAdView(adUnitId: AdConfig.LevelPlay_Banner_Ad_Id, config: configBuilder.build())
    ...
    @objc private func buttonShow() {
        guard let banner = bannerAdView else {
            logMessage("\(TAG) no banner to show")
            return
        }
        ...
        NSLayoutConstraint.activate([
            ...
            banner.heightAnchor.constraint(equalTo: adContainer.heightAnchor),
        ])
...
extension LevelPlayBannerVC: LPMBannerAdViewDelegate {
    func didLoadAd(with adInfo: LPMAdInfo) {
        ...
        DispatchQueue.main.async { [weak self] in self?.buttonShow() }
    }

    func didFailToLoadAd(withAdUnitId adUnitId: String, error: Error) {
        isLoading = false
        logMessage("\(TAG) didFailToLoadAd: \((error as NSError).code): \(error.localizedDescription)")
    }
    ...
    func didLeaveApp(with adInfo: LPMAdInfo) {
        logMessage("\(TAG) didLeaveApp")
    }

    func didExpandAd(with adInfo: LPMAdInfo) {
        logMessage("\(TAG) didExpandAd")
    }

    func didCollapseAd(with adInfo: LPMAdInfo) {
        logMessage("\(TAG) didCollapseAd")
    }
}
