//
//  LevelPlayRewardVideoVC.swift
//  AlxAdsDemo
//
//  LevelPlay 激励视频广告测试页面
//

import UIKit
import IronSource

class LevelPlayRewardVideoVC: BaseUIViewController {

    private let TAG = "LevelPlay-rewardVideo:"
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        navigationItem.title = NSLocalizedString("levelPlay_rewardVideo", comment: "")
        setupBackground()
        setupUI()
    }
    ...
    private func loadAd() {
        rewardedAd = LPMRewardedAd(adUnitId: AdConfig.LevelPlay_Reward_Ad_Id)
...
extension LevelPlayRewardVideoVC: LPMRewardedAdDelegate {

    func didLoadAd(with adInfo: LPMAdInfo) {
        logMessage("\(TAG) didLoadAd")
        logMessage("revenue = \(adInfo.adNetwork)")
        logMessage("revenue = \(adInfo.revenue)")
        logMessage("revenue = \(adInfo.encryptedCPM)")
        isLoading = false
    }

    func didFailToLoadAd(withAdUnitId adUnitId: String, error: Error) {
        let msg = "\((error as NSError).code): \(error.localizedDescription)"
        logMessage("\(TAG) didFailToLoadAd: \(msg)")
        isLoading = false
    }

    func didDisplayAd(with adInfo: LPMAdInfo) {
        logMessage("\(TAG) didDisplayAd")
    }

    func didRewardAd(with adInfo: LPMAdInfo, reward: LPMReward) {
        logMessage("\(TAG) didRewardAd: \(reward.name) x\(reward.amount)")
    }

    func didFailToDisplayAd(with adInfo: LPMAdInfo, error: Error) {
        logMessage("\(TAG) didFailToDisplayAd: \(error.localizedDescription)")
    }

    func didClickAd(with adInfo: LPMAdInfo) {
        logMessage("\(TAG) didClickAd")
    }

    func didCloseAd(with adInfo: LPMAdInfo) {
        logMessage("\(TAG) didCloseAd")
    }
}
