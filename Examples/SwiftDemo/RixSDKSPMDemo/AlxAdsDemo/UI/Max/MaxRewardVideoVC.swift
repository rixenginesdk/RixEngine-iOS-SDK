...
extension MaxRewardVideoVC: MARewardedAdDelegate {
    ...
    func didLoad(_ ad: MAAd) {
        ...
        logMessage("\(TAG) networkName = \(ad.networkName)")
        logMessage("\(TAG) revenue = \(ad.revenue * 1000)")
        logMessage("\(TAG) revenuePrecision = \(ad.revenuePrecision)")
        isLoading = false
    }
    ...
    func didFail(toDisplay ad: MAAd, withError error: MAError) {
        logMessage("\(TAG) didFail:\(error.code) \(error.description)")
    }

    func didRewardUser(for ad: MAAd, with reward: MAReward) {
        logMessage("\(TAG) didRewardUser")
    }
}
